...
const AdminQuoteRequests: React.FC = () => {
  ...
  const confirmAndMove = async (lead: Lead) => {
    ...
    const { error: bookingError } = await supabase.from('bookings').insert({
      ...
      final_total: lead.final_total,
      status: 'confirmed',
      source: lead.source || 'quote-request',
    });

    if (bookingError) { setSaving(false); flash('err', 'Could not create booking.'); return; }

    // Mark lead as confirmed
    await supabase.from('leads').update({ confirmed: true }).eq('id', lead.id);
                ...
                {expandedId === lead.id && (
                  <div className="border-t border-[#e0d4c0] p-5 bg-[#fbf7f0]">
                    <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                      ...
                      <div>
                        <p className="text-xs text-[#9a8d78] uppercase tracking-wider mb-1">Company</p>
                        <p className="text-[#2a2018] font-medium">{lead.company || '—'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-[#9a8d78] uppercase tracking-wider mb-1">Hours</p>
                        <p className="text-[#2a2018] font-medium">{lead.hours ? `${lead.hours} hours` : '—'}</p>
                      </div>
                      <div>
                      ...
                      {lead.service_addons && (
                        <div className="sm:col-span-2 lg:col-span-3">
                          <p className="text-xs text-[#9a8d78] uppercase tracking-wider mb-1">Service add-ons</p>
                          <p className="text-[#2a2018]">{lead.service_addons}</p>
                        </div>
                      )}
                      {lead.custom_addons && (
                        <div className="sm:col-span-2 lg:col-span-3">
                          <p className="text-xs text-[#9a8d78] uppercase tracking-wider mb-1">Custom add-ons</p>
                          <p className="text-[#2a2018]">{lead.custom_addons}</p>
                        </div>
                      )}
                      {lead.notes && (
                        <div className="sm:col-span-2 lg:col-span-3">
                          <p className="text-xs text-[#9a8d78] uppercase tracking-wider mb-1">Notes</p>
