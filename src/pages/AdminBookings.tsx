...
const getTeamSize = (guests: number) => {
  if (guests <= 25) return 2;
  if (guests <= 50) return 3;
  if (guests <= 75) return 4;
  if (guests <= 100) return 5;
  if (guests <= 150) return 6;
  if (guests <= 200) return 7;
  if (guests <= 300) return 8;
  if (guests <= 500) return 9;
  return 10;
};
...
const generateInvoice = (lead: Lead, calc: ReturnType<typeof calcTotals>) => {
  ...
  <table class="totals">
    ...
    <tr><td>Deposit due today (50%)</td><td class="r">${fmt(calc.total / 2)}</td></tr>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {leadsLoading ? (
          ...
          <div className="space-y-4">
            {leads.map(lead => {
              const displayTotal = parseMoney(lead.final_total || lead.estimated_total);
