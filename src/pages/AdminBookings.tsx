...
const AdminBookings: React.FC = () => {
  ...
  const fetchBookings = async () => {
    ...
    const { data, error: err } = await supabase
      .from('hat_bar_bookings').select('*').order('created_at', { ascending: false });
  ...
  const stats = useMemo(() => {
    const totalGuests = leads.reduce((s, l) => s + (l.guests || 0), 0) +
      bookings.reduce((s, b) => s + (b.guests || 0), 0);
    const totalEvents = leads.length + bookings.length;
    const amountBooked = leads.reduce((s, l) => s + parseMoney(l.final_total || l.estimated_total), 0);
    const amountPaid = leads.filter(l => l.deposit_received).reduce((s, l) => s + parseMoney(l.final_deposit || l.deposit), 0);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-[#e0d4c0] p-5">
            ...
            <p className="text-xs text-[#9a8d78] mt-0.5">{bookings.length} hat bar + {stats.confirmedLeads} quote requests</p>
          </div>
          <div className="bg-white rounded-xl border border-[#e0d4c0] p-5">
            <p className="text-xs uppercase tracking-wider text-[#7a6e5c]">Confirmed quote requests</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-[#e0d4c0] p-5">
            <p className="text-xs uppercase tracking-wider text-[#7a6e5c]">Total events</p>
            <p className="font-serif text-3xl text-[#2a2018]">{stats.totalEvents}</p>
            <p className="text-xs text-[#9a8d78] mt-0.5">{stats.confirmedLeads} confirmed</p>
          </div>
          <div className="bg-white rounded-xl border border-[#e0d4c0] p-5">
