...
const calcTotals = (
  ...
  const teamSize = teamSizeOverride || getTeamSize(guests);
...
const LeadEditModal: React.FC<{
  ...
  const [teamSizeOverride, setTeamSizeOverride] = useState<number | null>(lead.team_size_override || null);
  ...
  const calc = useMemo(() =>
    calcTotals(guests, hours, hatPricePerGuest, serviceAddons, customAddons, lineItems, taxable, taxRate, teamSizeOverride),
    [guests, hours, hatPricePerGuest, serviceAddons, customAddons, lineItems, taxable, taxRate, teamSizeOverride]
  );
  ...
  const save = async () => {
    ...
    const patch = {
      ...
      team_size_override: teamSizeOverride || null,
    };
    const { error } = await supabase.from('bookings').update(patch).eq('id', lead.id);
    setSaving(false);
              ...
              <div className="grid sm:grid-cols-3 gap-3 mb-3">
                ...
                <div>
                  <label className="block text-xs text-[#7a6e5c] mb-1">
                    Stylists & assistants
                    <span className="text-[#9a8d78] font-normal ml-1">(auto: {getTeamSize(guests)})</span>
                  </label>
                  <input type="number" min={1} max={20}
                    value={teamSizeOverride ?? ''}
                    onChange={e => setTeamSizeOverride(e.target.value ? parseInt(e.target.value) : null)}
                    placeholder={String(getTeamSize(guests))}
                    className="w-full rounded-lg border border-[#d8cbb4] px-3 py-2 text-sm outline-none focus:border-[#c9a36a]" />
                </div>
              </div>
...
const AdminBookings: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const amountBooked = leads.reduce((s, l) => {
      const hasCalcData = (l.hat_price_per_guest || 0) > 0;
      if (hasCalcData) {
        const c = calcTotals(l.guests || 0, l.hours || 3, l.hat_price_per_guest || 0,
          l.service_addons ? l.service_addons.split(',').map(x => x.trim()) : [],
          l.custom_addons ? l.custom_addons.split(',').map(x => x.trim()) : [],
          l.custom_line_items || [], l.taxable !== false, l.tax_rate || 0.0625, null);
        return s + c.total;
      }
      return s + parseMoney(l.final_total || l.estimated_total);
    }, 0);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        {leadsLoading ? (
          ...
          <div className="space-y-4">
            {leads.map(lead => {
              const hasCalcData = (lead.hat_price_per_guest || 0) > 0;
              const calc = calcTotals(
                ...
                lead.custom_line_items || [], lead.taxable !== false, lead.tax_rate || 0.0625, null
              );
              const displayTotal = hasCalcData ? calc.total : parseMoney(lead.final_total || lead.estimated_total);
                  ...
                  <div className="flex items-start justify-between flex-wrap gap-3">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      <div className="text-right">
                        ...
                        <p className="font-bold text-[#2a2018]">{fmt(displayTotal)}</p>
