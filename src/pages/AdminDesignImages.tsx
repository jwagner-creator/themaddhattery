  ...
  return (
    <div className="mb-12">
      ...
      <p className="text-[#cbbfa9] text-sm mb-5 max-w-2xl">
        ...
        instantly in the hat configurator — no code needed.
      </p>
      ...
      <div className="rounded-2xl bg-[#3a2e22] border border-[#4a3c2e] p-5 space-y-5">
                ...
                {newColorImage ? (
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={() => colorFileRef.current?.click()} className="text-xs text-[#c9a36a] hover:underline">
                      {uploadingColor ? 'Uploading…' : 'Replace'}
                    </button>
                  </div>
                ) : (
                  <button type="button" disabled={uploadingColor} onClick={() => colorFileRef.current?.click()}
                    ...
                    {uploadingColor ? 'Uploading…' : '+ Photo for this color'}
                  </button>
                )}
              </div>
            </div>
        ...
        <button type="button" disabled={saving} onClick={add}
          ...
          {saving ? 'Adding…' : 'Add base hat to configurator'}
        </button>
      </div>
  ...
  return (
    <div className="rounded-2xl overflow-hidden bg-[#3a2e22] border border-[#4a3c2e]">
      ...
      <div className="p-4">
        ...
        {/* Color swatches with individual photo upload */}
              ...
              {row.colors.map((c, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <label className="cursor-pointer text-[10px] text-[#c9a36a] hover:underline flex-shrink-0">
                    {uploadingColorIdx === i ? 'Uploading…' : c.image ? 'Replace' : '+ Photo'}
...
const SlotCard: React.FC<{
  ...
  const upload = async (file: File) => {
    ...
    setBusy(false);
    if (ok) { onChanged(slot.key, publicUrl); flash('ok', `Updated "${slot.label}". The new photo is now live.`); }
  ...
  const pickFromGallery = async (galleryUrl: string) => {
    ...
    setBusy(false);
    if (ok) { onChanged(slot.key, galleryUrl); flash('ok', `Swapped "${slot.label}" to the chosen gallery photo.`); }
