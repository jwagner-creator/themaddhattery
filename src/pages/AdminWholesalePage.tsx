...
const AdminWholesalePage: React.FC = () => {
  ...
  // New retailer form
  const [newRetailer, setNewRetailer] = useState({ business_name: '', email: '', password: '' });
  ...
  const addRetailer = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('wholesale_retailers').insert({
      ...
      email: newRetailer.email.toLowerCase(),
      password_hash: newRetailer.password,
      active: true,
    });
  ...
  const addProduct = async (e: React.FormEvent) => {
    ...
    setNewProduct({ category: 'feathers', name: '', description: '', price: '', unit: 'each', variations: '', image: '', in_stock: true });
        ...
        {tab === 'applications' && (
          <div>
            ...
            <div className="bg-white rounded-xl border border-[#e0d4c0] p-6 mb-8">
              <h2 className="font-serif text-xl text-[#2a2018] mb-4">Create retailer account</h2>
              <form onSubmit={addRetailer} className="grid sm:grid-cols-4 gap-3">
                <input value={newRetailer.business_name} onChange={e => setNewRetailer(p => ({ ...p, business_name: e.target.value }))}
                  placeholder="Business name" required
                  className="rounded-lg border border-[#d8cbb4] px-4 py-2.5 outline-none focus:border-[#c9a36a]" />
                <input type="email" value={newRetailer.email} onChange={e => setNewRetailer(p => ({ ...p, email: e.target.value }))}
                  placeholder="Email address" required
                  className="rounded-lg border border-[#d8cbb4] px-4 py-2.5 outline-none focus:border-[#c9a36a]" />
                <input value={newRetailer.password} onChange={e => setNewRetailer(p => ({ ...p, password: e.target.value }))}
                  placeholder="Set a password" required
                  className="rounded-lg border border-[#d8cbb4] px-4 py-2.5 outline-none focus:border-[#c9a36a]" />
                <button type="submit" disabled={savingRetailer}
                  className="rounded-full bg-[#c9a36a] hover:bg-[#b8915a] text-[#2a2018] font-semibold py-2.5 transition-colors disabled:opacity-50">
                  {savingRetailer ? 'Creating…' : 'Create account'}
                </button>
              </form>
            </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border border-[#e0d4c0] p-6 mb-8">
              ...
              <form onSubmit={addProduct} className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  <label className="block text-xs text-[#7a6e5c] mb-1">Product photo</label>
                  <input ref={fileRef} type="file" accept="image/*" onChange={e => setProductImage(e.target.files?.[0] || null)} className="hidden" />
                  <div onClick={() => fileRef.current?.click()}
                    className="cursor-pointer rounded-lg border-2 border-dashed border-[#d8cbb4] hover:border-[#c9a36a] px-4 py-4 text-center transition-colors">
                    {productImage ? (
                      <p className="text-sm text-[#2a2018]">{productImage.name}</p>
                    ) : (
                      <p className="text-sm text-[#5b5043]">Click to upload a photo</p>
                    )}
                  </div>
                </div>
                ...
                <div className="sm:col-span-2">
                  <button type="submit" disabled={saving}
                    ...
                    {saving ? (uploading ? 'Uploading photo…' : 'Adding…') : 'Add product'}
                  </button>
                </div>
              </form>
            </div>
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    {product.description && <div className="text-xs text-[#9a8d78]">{product.description}</div>}
                                  </div>
                                </div>
                              </td>
                              <td className="px-4 py-3 text-[#2a2018] font-medium">{money(product.price)}</td>
                              <td className="px-4 py-3 text-[#5b5043]">{product.unit}</td>
                              <td className="px-4 py-3 text-[#5b5043] max-w-[200px]">{product.variations || '—'}</td>
                              <td className="px-4 py-3">
