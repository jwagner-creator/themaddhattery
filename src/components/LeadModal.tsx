...
const ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imh5c3RsZWhqd3BhZ2NrdG95b2lhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODIyNjY5MDUsImV4cCI6MjA5Nzg0MjkwNX0.XE3XIxkuJLl5UNyD7L8RH7knNK8_sSZv1qdEg1a0zvs';
    ...
    try {
      ...
      try {
        await fetch(`${EDGE_URL}/event-quote-email`, {
          ...
          body: JSON.stringify({
            lead: { name, email, phone, event_type: eventTypeLabel, event_date: eventDate, guests,
              estimated_total: breakdown ? money(breakdown.total) : null,
              deposit: breakdown ? money(breakdown.deposit) : null, notes },
          }),
        });
      } catch { /* non-blocking */ }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#f6efe4] rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <button onClick={onClose} className="absolute top-4 right-4 text-[#5b5043] hover:text-[#2a2018] text-2xl leading-none" aria-label="Close">×</button>
        <div className="p-7 sm:p-8">
          {status === 'done' ? (
            <div className="text-center py-8">
              <div className="mx-auto w-14 h-14 rounded-full bg-[#c9a36a] flex items-center justify-center mb-5">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#2a2018" strokeWidth="3"><path d="M20 6L9 17l-5-5" /></svg>
              </div>
              <h3 className="font-serif text-2xl text-[#2a2018] mb-2">You're on the list!</h3>
              <p className="text-[#5b5043] mb-6">We'll be in touch shortly with your custom quote. Want to lock in a time now?</p>
            ...
            <>
              <p className="text-xs uppercase tracking-[0.25em] text-[#b8915a] mb-2">Get your quote</p>
              <h3 className="font-serif text-2xl text-[#2a2018] mb-1">Send me this quote &amp; details</h3>
              {breakdown && (
                <p className="text-[#5b5043] mb-5">
                  Estimated total <span className="font-semibold text-[#2a2018]">{money(breakdown.total)}</span> · Deposit {money(breakdown.deposit)}
                </p>
              )}
              <form onSubmit={submit} className="space-y-3">
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="Anything else we should know? (venue, theme, custom add-ons...)" rows={3}
                ...
                <label className="flex items-start gap-2 text-sm text-[#5b5043]">
                  ...
                  <span>Text me about my event. Msg &amp; data rates may apply. Reply STOP to unsubscribe.</span>
                </label>
