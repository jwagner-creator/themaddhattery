...
const LeadModal: React.FC<LeadModalProps> = ({
  ...
  // Prefill notes & event date from the quote builder when the modal opens.
  useEffect(() => {
    if (open) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const tags = [
        ...
        eventTypeLabel,
        `${guests}-guests`,
        breakdown ? `est-${money(breakdown.total)}` : 'no-estimate',
      ];
      if (eventDate) tags.push(`event-date:${eventDate}`);
      if (sizes.length) tags.push(`sizes:${sizes.join('/')}`);

      const messageParts: string[] = [];
      if (eventDate) messageParts.push(`Event date: ${eventDate}`);
      messageParts.push(`Event type: ${eventTypeLabel}`);
      messageParts.push(`Guests: ${guests}`);
      if (sizes.length) messageParts.push(`Hat sizes: ${sizes.join(', ')}`);
      if (breakdown) {
        messageParts.push(`Estimated total: ${money(breakdown.total)}`);
        messageParts.push(`Deposit: ${money(breakdown.deposit)}`);
      }
      ...
      await fetch('https://famous.ai/api/crm/6a3626102bd450af612d0a20/subscribe', {
        ...
        body: JSON.stringify({
          ...
          fields: {
            ...
            hat_sizes: sizes.length ? sizes.join(', ') : undefined,
            estimated_total: breakdown ? money(breakdown.total) : undefined,
            deposit: breakdown ? money(breakdown.deposit) : undefined,
      ...
      // Still show success — lead intent captured; never block the user
      setStatus('done');
    }
  };
