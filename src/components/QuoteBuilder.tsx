...
import {
  EVENT_TYPES, BUDGET_TIERS, HOURS_OPTIONS, SERVICE_ADDONS, CUSTOM_ADDONS,
  STYLIST_HOURLY_RATE, TRAVEL_RATE_PER_STAFF, computeQuote, getTeamSize,
  getBranderCount, recommendedHours, money, BOOKING_URL, QuoteState
} from '@/data/quoteData';
      ...
      <div className="max-w-6xl mx-auto px-5">
        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
          <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 space-y-7">
            ...
            {/* 05 Event date + location + company + flexible */}
            ...
            <Section step="06" title="Service add-ons">
              <div className="space-y-2">
                {SERVICE_ADDONS.map(a => {
                  ...
                  const displayPrice = a.perStaff
                    ? money(a.price * getTeamSize(state.guests))
                    : money(a.price);
                  const priceLabel = a.perStaff
                    ? `${money(a.price)} × ${getTeamSize(state.guests)} staff = ${displayPrice}`
                    : displayPrice;
                  return (
                    <button key={a.id} onClick={() => toggle('serviceAddons', a.id)}
                      ...
                      <span className="flex-1 pr-3">
                        ...
                        {active && a.id === 'branding' && state.guests > 0 && (
            ...
            <Section step="07" title="Custom add-ons" sub="Priced per guest and added straight into your estimated total.">
              <div className="space-y-2">
                {CUSTOM_ADDONS.filter(a => !a.maxGuests || state.guests <= a.maxGuests).map(a => {
                  ...
                  return (
                    <button key={a.id} onClick={() => toggle('customAddons', a.id)}
                      ...
                      <span className="text-sm text-[#7a6e5c] text-right whitespace-nowrap">
                        <span className="block">{a.priceLabel}</span>
                        {active && <span className="block text-xs font-semibold text-[#2a2018]">+{money(a.pricePerGuest * state.guests)}</span>}
                      </span>
                    </button>
                  );
                })}
                {CUSTOM_ADDONS.some(a => a.maxGuests && state.guests > a.maxGuests) && (
                  <p className="text-xs text-[#9a8d78] mt-1">
                    Some packages are only available for smaller groups and are hidden based on your guest count.
                  </p>
                )}
              </div>
            </Section>
          ...
          <div className="lg:sticky lg:top-24">
            <div className="bg-[#2a2018] text-[#f3ead9] rounded-2xl shadow-xl p-6 sm:p-7">
              ...
              <div className="space-y-3 text-sm">
                ...
                {hasBranding && (
                  <Row label={`Branding station${breakdown.branderCount > 0 ? ` (${breakdown.branderCount} brander${breakdown.branderCount > 1 ? 's' : ''})` : ''}`} value={money(150)} />
                )}
                {hasTravel && (
                  <Row label={`Travel (${money(TRAVEL_RATE_PER_STAFF)} × ${breakdown.teamSize} staff)`} value={money(breakdown.travelTotal)} />
                )}
                {breakdown.customAddonsTotal > 0 && (
                  <Row label={`Custom add-ons (× ${state.guests})`} value={money(breakdown.customAddonsTotal)} />
                )}
                {breakdown.taxTotal > 0 && (
                  <Row label="Sales tax (6.25%)" value={money(breakdown.taxTotal)} />
                )}
              </div>
