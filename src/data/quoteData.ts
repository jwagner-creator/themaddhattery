...
export const BOOKING_URL =
  'https://famous.ai/api/crm/6a3626102bd450af612d0a20/calendar/public?calendarId=1ff4bed7-a606-4971-bf64-ca7f247def37&view=booking';
...
// Only the stylist fee and the branding station fee apply.
...
export interface BudgetTier {
  id: string;
  range: string;
  value: number; // representative per-guest hat cost used for the estimate
  title: string;
  description: string;
  brandable?: boolean;
}
...
export const BUDGET_TIERS: BudgetTier[] = [
  ...
  },
  {
    ...
    description: 'Premium Australian wool felt — best option for branding and personalization.',
    brandable: true,
  },
];
...
// Hat sizes guests can request for their event.
...
export interface CustomAddOn {
  id: string;
  label: string;
  pricePerGuest: number; // $ per guest
  priceLabel: string;
}
...
// Stylist + assistant team size based on guest count (scales up to 8).
...
export function computeQuote(state: QuoteState): QuoteBreakdown {
  const hatTier =
    BUDGET_TIERS.find((t) => t.id === state.budgetTierId) || BUDGET_TIERS[0];
